...
3、执行JS
4、等待固定时间
5、等待网页加载完毕，一直等待，无终止条件
6、等待网页加载完毕，30 秒作为最长等待时间
7、自定义显式等待，等待网页标题包含预期标题
8、封装获取页面元素状态（是否可见、是否可用）的方法
9、封装选择下拉框的选项方法：
10、页面元素点击可以直接用click方法。为了方便调试，最好要先判断该元素在页面上是否存在。
11、点击页面元素并且等待网页加载的clickAndWait方法如下：
12、页面输入通常使用sendKeys方法
13、 Frame切换--切换到默认网页
14、使用ID或Name属性值来切换到新的Frame
15、通过index定位切换Frame
16、通过其他方式定位切换Frame
17、通过网页内容定位切换Frame
18、通过名称切换到新窗口
19、 切换到另一个窗口(适合一共叧有2个窗口的情况)
20、多个窗口的情况下，可以判断窗口标题或URL或网页源码是否满足条件，如果满足,通过窗口句柄切换到新窗口
21、 断言页面元素出现--判断指定的页面元素是否出现
22、判断弹出框是否出现
23、获得弹出框中内容，并且关闭弹出框，默认点击“确定”
24、 页面断言文本等于预期值
25、封装断言属性值等于预期值的方法
26、 断言复选框或单选按钮当前被选中
27、 断言复选框或单选按钮当前未被选中
28、 断言下拉框或列表框的当前选项等于预期文本
29、断言下拉框或列表框的所有选项中包含一些预期文本
30、 断言下拉框或列表框的当前选项可以多选
31、 断言下拉框或列表框的当前选项丌可以多选，只能单选
32、断言下拉框或列表框的多个当前选项中包含预期文本
33、断言下拉框或列表框的所有选项个数正确
34、 断言下拉框或列表框的当前选项个数正确
35、获取系统当前是时间
*/
...
public class Util {
	...
	public static void takeScreenshot(String sTestCaseName) {
		...
		try {
			FileUtils.copyFile(file, new File(Constants.SCREENSHOT + sTestCaseName + "/" + sTestCaseName + " # " +
...
///10、页面元素点击可以直接用click方法。为了方便调试，最好要先判断该元素在页面上是否存在。步骤如下：–1、判断该页面元素是否存在和可用–2、调用click方法–3、如果网页有更新，
